...
const INITIAL_STATE: LoadState = {
  phase: "idle",
  progress: 0,
  message: "Opening the Hokage's databook v4.0…",
  fromCache: false,
  error: null,
  counts: {
    characters: 0,
...
export function useDatabook(): Databook {
  ...
  const run = useCallback(async (force: boolean) => {
    if (force) {
      ...
      setState({ ...INITIAL_STATE, phase: "fetching", message: "Re-summoning the databook v4.0…" });
    }
    ...
    try {
      ...
      setData(dataset);
      setState((prev) => ({ ...prev, phase: "ready", progress: 1, message: "Databook v4.0 online", fromCache: false }));
    } catch (err) {
      setState((prev) => ({
        ...prev,
